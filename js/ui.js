...
function initSuperClock(){
  const face = document.querySelector(".clock-face");
  if(!face) return;

  face.querySelectorAll(".tick").forEach(el=>el.remove());

  for(let i=0;i<60;i++){
    const t = document.createElement("div");
    t.className = i % 5 === 0 ? "tick big" : "tick";
    t.style.transform =
...
initSuperClock();
updateSuperClock();
setInterval(updateSuperClock,1000);

function fitSuperCard(){
...
fitSuperCard();
window.addEventListener("resize", fitSuperCard);
new ResizeObserver(fitSuperCard)
.observe(document.querySelector(".bottom-section"));
